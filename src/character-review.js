...
const entries=[...residents,{id:'player',name:'あなた'}].map(data=>{const root=new T.Group(),body=new T.Group(),workProp=new T.Group();root.add(body);body.add(workProp);workProp.visible=false;scene.add(root);const n={...data,root,body,workProp,eyes:[]};return {n,visual:upgradeResident(n)};});
let selected='all',angle=.06,clock=1,gestureUntil=0,previous=performance.now();
const descriptions={player:'あなた · 自分で記憶を選ぶ身体。胸のハッチの中に、三つのブロック。',all:'左から Mia · Ren · Tomo · Shell · あなた',mia:'Mia · 好奇心旺盛な聞き手。ピーチ色の外装とマフラー。',ren:'Ren · 理論派の研究者。メガネと紺のベスト。',tomo:'Tomo · 試して学ぶ開発者。猫耳、しっぽ、試作道具。',shell:'Shell · 静かな街の管理者。緑の外装と二枚の葉。'};
function layout(){entries.forEach(({n},i)=>{n.root.visible=selected==='all'||selected===n.id;n.root.position.set(selected==='all'?(i-2)*1.5:0,0,0);});$('description').textContent=descriptions[selected];}
...
function frame(now){requestAnimationFrame(frame);const dt=Math.min((now-previous)/1000,.05);previous=now;if(document.hidden)return;clock+=dt;for(const {n,visual} of entries)visual.update(clock,$('expression').value,clock<gestureUntil&&n.root.visible);const distance=(selected==='all'?Math.max(8,12/camera.aspect):Math.max(3.8,3/camera.aspect))*100/Number($('zoom').value);camera.position.set(Math.sin(angle)*distance,selected==='all'?2.7:1.42,Math.cos(angle)*distance);camera.lookAt(0,1.05,0);renderer.render(scene,camera);}

...
import {discoveries,visitPlaces,discoveryVisitSpot} from './discovery-rules.js';
...
export function navigationAudit(world){
  ...
  for(const p of discoveries)goals.push([p.name+' / '+p.id,{x:p.x,z:p.z}]);
  for(const place of visitPlaces)for(const id of ['tomo','mia','ren','shell'])goals.push([place+' visit / '+id,discoveryVisitSpot(place,id)]);

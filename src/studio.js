...
export function createStudio(world){
 ...
 const posterStand=box(0x755f50,-7.2,1.1,3.1,.15,2.2,.2);screen(-7.2,2.2,3.2,'published');posterStand.visible=false;
 return {furniture,setRoom(room){for(const [id,objects] of Object.entries(rooms))for(const o of objects)o.visible=id===room&&!o.userData.replaced;},update(state){posterStand.visible=!!state.projects?.some(p=>p.kind==='poster'&&p.published);for(const d of displays){if(d.id==='library')continue;if(d.id==='published')d.mesh.visible=posterStand.visible;const p=d.id==='published'?state.projects?.find(p=>p.kind==='poster'&&p.published):state.projects?.find(p=>p.id===d.id);const key=JSON.stringify([p?.revision,p?.phase]);if(key===d.key)continue;d.key=key;drawArtifact(d.canvas,p);d.texture.needsUpdate=true;}}};
}

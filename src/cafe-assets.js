...
export async function upgradeCafe(world){
 ...
 return {group,assetCount:names.length,update(t){for(const s of sway)s.object.rotation.z=Math.sin(t*1.2+s.phase)*s.amount;for(const s of steam){const p=(t*.23+s.phase)%1;s.object.position.set(s.x+Math.sin(t+s.phase)*.035,1.08+p*.45,s.z);s.object.material.opacity=(1-p)*.12;s.object.scale.setScalar(.10+p*.15);}for(const l of lights)l.light.intensity=l.power*(1+Math.sin(t*2.4)*.008);}};

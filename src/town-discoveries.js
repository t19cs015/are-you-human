...
import {craftMaterial,createCraftFountain,createLanternSkiff,dressChime,dressWaterwheel,craftRing,craftBox} from './town-craft.js';
...
export function createTownDiscoveries(world){
 ...
 const materials=new Map(),geos={box:new RoundedBoxGeometry(1,1,1,2,.055),sphere:new T.SphereGeometry(1,20,12),cylinder:new T.CylinderGeometry(1,1,1,32)},turning=[],bells=[],windows=[],ripples=[],boats=[],stems=[];
 const timber=new Set([0xba9276,0xc2a187,0xb89b7a,0xad977b,0xb39f80,0x7b9486]),natural=new Set([0x879891,0xa8ada0,0x85ab91,0x85a48d,0x749d85,0x7e9e82,0x71937a]);
 const mat=(color,glow=0)=>{const key=color+':'+glow;if(!materials.has(key))materials.set(key,craftMaterial(color,timber.has(color)?'wood':natural.has(color)?'matte':color===0xd8b982?'metal':'ceramic',glow));return materials.get(key);};
 ...
 function chime(id,x,z,width=1.5){
  dressChime(staticRoot,x,z,width);
  ...
  for(let i=0;i<5;i++){const b=new T.Group();b.position.set(x+(i-2)*width/6,2.85,z);group.add(b);box(0x688c86,0,-.2,0,.012,.4,.012,b);part('cylinder',[0xe2b79e,0xb6cfc1,0xe6d6af][i%3],0,-.58-i%3*.06,0,.05,.45+i%3*.12,.05,b);for(const y of [-.36-i%3*.06,-.8-i%3*.12])craftRing(b,0xc6b084,0,y,0,.053,.012,true);bells.push({id,b,i});}
 }
 function wheel(id,x,y,z,r=.43){const g=new T.Group();g.position.set(x,y,z);group.add(g);ring(0xd8b982,0,0,0,r,g);sphere(0x86aaa0,0,0,0,.12,g);for(let i=0;i<3;i++){const b=box(0xd8b982,0,0,0,r*1.8,.06,.065,g);b.rotation.z=i*Math.PI/3;}turning.push({id,g,speed:.18});return g;}
 ...
 const waterwheel=wheel('pump',-20.25,1.02,36.8,1.26);waterwheel.name='River waterwheel';turning.at(-1).speed=.4;turning.at(-1).boostSpeed=1.3;
 dressWaterwheel(waterwheel);
 const flume=new T.Mesh(new T.PlaneGeometry(2.48,1.75,24,12),river.material);flume.rotation.x=-Math.PI/2;flume.position.set(-20.25,.178,36.8);group.add(flume);
 for(const side of [-1,1])box(0x94a697,-20.25+side*1.26,.18,36.8,.08,.24,1.8);
 ...
 const fountain=createCraftFountain(group,fx,fz);obstacle(fx,fz,1.23,1.23);
 ...
 craftBox(staticRoot,0x68867f,0,.83,77.813,1.38,.64,.04);
 for(const x of [-.61,.61])for(const y of [.59,1.07])sphere(0xcbb88d,x,y,77.848,.026);
 for(const x of [-.27,0,.27])craftBox(staticRoot,0xd4b887,x,1.31,77.45,.045,.012,.16,'metal');
 ...
 for(let i=0;i<7;i++){const b=createLanternSkiff();group.add(b);const fadeMaterials=[];if(i===0)b.traverse(o=>{if(o.isMesh){o.material=o.material.clone();if(!o.material.transparent)o.renderOrder=2;fadeMaterials.push({material:o.material,opacity:o.material.opacity,transparent:o.material.transparent,depthWrite:o.material.depthWrite});}});boats.push({b,i,fadeMaterials});}
 ...
 return {group,touch,focus(p){target=p;},update(time,city){
  ...
  for(const {id,g,speed,boostSpeed=8} of turning){const boost=pulse&&activeTouch===id;g.rotation.z+=dt*(boost&&id!=='telescope'?boostSpeed:speed)*(id==='pump'?(e?.waterOpen?1:.18):1);if(id==='clock')g.rotation.z+=(e?.clockOffset||0)*dt*.2;}
  ...
  fountain.update(time,pulse&&activeTouch==='fountain');
  ...
  boats.forEach(({b,i,fadeMaterials})=>{if(i===0){const p=lanternJourney(e?.boatAt===undefined?0:city.clock-e.boatAt);b.position.set(p.x,-.19,p.z);b.rotation.y=p.angle;b.visible=p.progress<1;fadeMaterials.forEach(({material:m,opacity,transparent,depthWrite})=>{
    // Solid timber writes depth; glass keeps its original transparency while
    // every part fades together at the mist gate.
    const fading=p.opacity<.999,nextTransparent=transparent||fading;
    if(m.transparent!==nextTransparent){m.transparent=nextTransparent;m.depthWrite=fading?false:depthWrite;m.needsUpdate=true;}m.opacity=opacity*p.opacity;
  });}else{const row=canalRows[i%2];b.position.set(((time*.55+i*12)%76)-38,-.19,row+Math.sin(i)*.35);b.rotation.y=Math.PI/2;}b.position.y+=Math.sin(time*1.8+i)*.018;b.rotation.z=Math.sin(time*1.1+i)*.025;b.userData.flame.scale.y=.09+Math.sin(time*9+i)*.008;});

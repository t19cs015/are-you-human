...
test('HTTP boundary: isolated sessions, safe files, origin, validation and mock live conversation',async()=>{
 ...
 try{
  ...
  for(const id of ['mia','ren','tomo','shell']){const model=await fetch(base+`/assets/characters/${id}.glb`);assert.equal(model.status,200);assert.equal(model.headers.get('content-type'),'model/gltf-binary');assert.equal((await request(`/art/characters/${id}.blend`)).status,404);}
  assert.equal((await fetch(base+'/character-review.html')).status,200);assert.equal((await request('/assets/characters/unknown.glb')).status,404);

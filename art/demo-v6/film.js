...
import {v6Script} from './script.mjs';
...
const replay=await response.json();let script=v6Script(replay),manifest=null;
...
function pose(t,city){
  ...
  if(t<6.2){
    ...
    for(const n of npcs){const p=opening[n.id],notice=smooth((t-p.at)/.6),lean=Math.max(0,Math.sin((t-p.at)*6))*Math.exp(-Math.max(0,t-p.at)*2)*.07;
      n.root.position.set(p.x+(n.id==='tomo'?.12*Math.sin(t*2):0),.09+(t>p.at?lean:0),p.z+(n.id==='tomo'?smooth((t-3.1)/1.9)*.7:0));
  ...
  else if(t<53.25){ground(.35,86.85,[0,2.9,97],58);}
  ...
  for(const n of npcs){
    ...
    if(t<6.2&&t>opening[n.id].at+.35)visuals.get(n.id).wave(30+opening[n.id].at+.35);
    visuals.get(n.id).update(time,t<4?'surprised':t>=16.65&&t<18.85?'confused':'happy',speaking,false,syncing);

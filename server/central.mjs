...
export function centralSnapshot(world){
  ...
  if(m?.active)return {memory:{stage:m.stage,cycle:m.cycle,nextSync:m.nextSync,shared:m.shared,preserved:m.preserved,windTuned:!!m.exploration?.windTuned,waterOpen:!!m.exploration?.waterOpen,frontier:{visited:!!m.exploration?.visited?.margin,boatSent:m.exploration?.boatAt!==undefined,replyObserved:!!m.exploration?.shoreRead,windowRestored:!!m.exploration?.borderShared}},modelEnabled:g.modelEnabled,energy:Math.floor(g.energy),water:Math.floor(g.water),
    residents:Object.entries(world.city.tasks).map(([id,t])=>({name:world.agents[id].name,job:t.label})),
    deliveredRecords:[...(m.exploration?.centralMemory?[{by:'あなた',text:m.exploration.centralMemory.text}]:[]),...Object.entries(m.sharedMemories||{}).flatMap(([id,memories])=>memories.map(memory=>({by:world.agents[id].name,text:memory.text})))],

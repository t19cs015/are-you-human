...
export function centralSnapshot(world){
  ...
  if(m?.active)return {memory:{stage:m.stage,cycle:m.cycle,nextSync:m.nextSync,shared:m.shared,preserved:m.preserved,windTuned:!!m.exploration?.windTuned,waterOpen:!!m.exploration?.waterOpen},modelEnabled:g.modelEnabled,energy:Math.floor(g.energy),water:Math.floor(g.water),

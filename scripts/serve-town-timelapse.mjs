...
const server=http.createServer(async(req,res)=>{
  try{
    ...
    const url=new URL(req.url,'http://'+req.headers.host),path=url.pathname,target=path.startsWith('/arrival/render/')?'arrival':'timelapse',route=target==='arrival'?path.slice(8):path;
    const framesExpected=target==='arrival'?1800:720,renderBuild=target==='arrival'?root+'data/arrival-film/':build;
    ...
    if(req.method==='POST'&&route==='/render/frame'){
      ...
      const samples=target==='arrival'?[0,60,120,180,270,360,450,540,630,720,810,900,1080,1190,1230,1350,1440,1560,1690,1770]:[0,90,180,270,360,450,540,630,690,719];

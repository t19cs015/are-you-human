...
import {prepareV8,makeDrawerSociety,localize,build} from './prepare-demo-v8.mjs';
...
const server=http.createServer(async(req,res)=>{try{
  ...
  if(req.method==='POST'){
    ...
    if(path==='/prepare'){preparing??=prepareV8(data.colliders).finally(()=>preparing=null);const replay=await preparing;drawer=makeDrawerSociety(replay);return json(res,200,replay);}
  ...
  const mapped={'/':'art/demo-v8/studio.html','/replay.json':'data/film-v8/replay.json','/audio-manifest.json':'data/film-v8/audio-manifest.json','/master.wav':'data/film-v8/master.wav'}[path];
  const allowed=/^\/(src|assets|node_modules\/three|art\/demo-v8)\/[a-zA-Z0-9_./-]+$/.test(path);
...
server.listen(4180,'127.0.0.1',()=>console.log('V8 production studio: http://127.0.0.1:4180/'));

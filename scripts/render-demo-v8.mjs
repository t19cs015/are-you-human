...
// Usage: node --experimental-websocket scripts/render-demo-v8.mjs --preview
...
const build=fileURLToPath(new URL('../data/film-v8/',import.meta.url));await mkdir(build,{recursive:true});
const riverOnly=process.argv.includes('--river'),preview=process.argv.includes('--preview')||riverOnly,profile=await mkdtemp(join(tmpdir(),'ayh-v8-chrome-'));

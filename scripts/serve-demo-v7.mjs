...
import {prepareV7,makeDrawerSociety,localize,build} from './prepare-demo-v7.mjs';
...
const server=http.createServer(async(req,res)=>{try{
  ...
  if(req.method==='POST'){
    ...
    if(path==='/prepare'){preparing??=prepareV7(data.colliders).finally(()=>preparing=null);const replay=await preparing;drawer=makeDrawerSociety(replay);return json(res,200,replay);}
  ...
  const mapped={'/':'art/demo-v7/studio.html','/replay.json':'data/film-v7/replay.json','/audio-manifest.json':'data/film-v7/audio-manifest.json','/master.wav':'data/film-v7/master.wav'}[path];
  const allowed=/^\/(src|assets|node_modules\/three|art\/demo-v7)\/[a-zA-Z0-9_./-]+$/.test(path);
...
server.listen(4180,'127.0.0.1',()=>console.log('V7 production studio: http://127.0.0.1:4180/'));

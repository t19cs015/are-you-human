...
import {prepareV6,makeDrawerSociety,localize,build} from './prepare-demo-v6.mjs';
...
const server=http.createServer(async(req,res)=>{try{
  ...
  if(req.method==='POST'){
    ...
    if(path==='/prepare'){preparing??=prepareV6(data.colliders).finally(()=>preparing=null);const replay=await preparing;drawer=makeDrawerSociety(replay);return json(res,200,replay);}
  ...
  const mapped={'/':'art/demo-v6/studio.html','/replay.json':'data/film-v6/replay.json','/audio-manifest.json':'data/film-v6/audio-manifest.json','/master.wav':'data/film-v6/master.wav'}[path];
  const allowed=/^\/(src|assets|node_modules\/three|art\/demo-v6)\/[a-zA-Z0-9_./-]+$/.test(path);
...
server.listen(4180,'127.0.0.1',()=>console.log('V6 production studio: http://127.0.0.1:4180/'));
